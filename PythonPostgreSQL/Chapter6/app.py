...
def prompt_create_poll(connection):
    ...
    test.create_poll(connection, poll_title, poll_owner, options)
...
def prompt_vote_poll(connection):
    ...
    poll_options = test.get_poll_details(connection, poll_id)
    _print_poll_options(poll_options)

    option_id = int(input("Enter option you'd like to vote for: "))
    username = input("Enter the username you'd like to vote as: ")
    test.add_poll_vote(connection, username, option_id)


def _print_poll_options(poll_with_options: List[test.PollWithOption]):
...
def show_poll_votes(connection):
    ...
    try:
        ...
        poll_and_votes = test.get_poll_and_vote_results(connection, poll_id)
...
def randomize_poll_winner(connection):
    ...
    winner = test.get_random_poll_vote(connection, option_id)
